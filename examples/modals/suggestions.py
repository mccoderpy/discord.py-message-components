...
class ModalSuggestions(commands.Cog):
    ...
    async def new_suggestion(self, interaction: ComponentInteraction, button):
        ...
        try:
            ...
            url_msg = await inter.channel.send(content) if content else None
            if url_msg:
                await mes.edit(
                    components=[
                        [
    ...
    async def delete_suggestion(self, interaction: ComponentInteraction, button):
        if (
            ...
            != int(interaction.message.embeds[0].author.url.split('/')[-1])
            and not interaction.author.permissions_in(
                interaction.channel
            ).manage_messages
        ):
            return
        ...
        if match := re.search(
            r'(?P<url_msg_id>[\d]+)',
            list(interaction.message.all_buttons)[0].custom_id,
        ):
            url_msg = discord.PartialMessage(
                channel=interaction.channel, id=int(match['url_msg_id'])
            )
        else:
            url_msg = None
    ...
    async def edit_suggestion(self, interaction: ComponentInteraction, button):
        ...
        if re.search(r'(\[see video below])', old_description):
            if match := re.search(
                ...
                url_msg = await discord.PartialMessage(
                    channel=interaction.channel, id=int(match['url_msg_id'])
                ).fetch()
