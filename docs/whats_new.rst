...
- Fix ``fail_if_not_exists`` causing certain message references to not be usable within :meth:`abc.Messageable.send` and :meth:`Message.reply` (:old-issue:`6726`)
- Fix :meth:`Guild.chunk` hanging when the user left the guild. (:old-issue:`6730`)
- Fix loop sleeping after final iteration rather than before (:old-issue:`6744`)
...
- Add support for stage channels via :class:`StageChannel` (:old-issue:`6602`, :old-issue:`6608`)
- Add support for :attr:`MessageReference.fail_if_not_exists` (:old-issue:`6484`)
...
- Add support for voice regions in voice channels via :attr:`VoiceChannel.rtc_region` (:old-issue:`6606`)
- Add support for :meth:`PartialEmoji.url_as` (:old-issue:`6341`)
- Add :attr:`MessageReference.jump_url` (:old-issue:`6318`)
- Add :attr:`File.spoiler` (:old-issue:`6317`)
- Add support for passing ``roles`` to :meth:`Guild.estimate_pruned_members` (:old-issue:`6538`)
- Allow callable class factories to be used in :meth:`abc.Connectable.play` (:old-issue:`6478`)
- Add a way to get mutual guilds from the client's cache via :attr:`User.mutual_guilds` (:old-issue:`2539`, :old-issue:`6444`)
- :meth:`PartialMessage.edit` now returns a full :class:`Message` upon success (:old-issue:`6309`)
- Add :attr:`RawMessageUpdateEvent.guild_id` (:old-issue:`6489`)
- :class:`AuditLogEntry` is now hashable (:old-issue:`6495`)
- :class:`Attachment` is now hashable
- Add :attr:`Attachment.content_type` attribute (:old-issue:`6618`)
- Add support for casting :class:`Attachment` to :class:`str` to get the URL.
- Add ``seed`` parameter for :class:`Colour.random` (:old-issue:`6562`)
    - This only seeds it for one call. If seeding for multiple calls is desirable, use :func:`random.seed`.

- Add a :func:`utils.remove_markdown` helper function (:old-issue:`6573`)
- Add support for passing scopes to :func:`utils.oauth_url` (:old-issue:`6568`)
- |commands| Add support for ``rgb`` CSS function as a parameter to :class:`ColourConverter <ext.commands.ColourConverter>` (:old-issue:`6374`)
- |commands| Add support for converting :class:`StoreChannel` via :class:`StoreChannelConverter <ext.commands.StoreChannelConverter>` (:old-issue:`6603`)
...
- |commands| Add :attr:`Context.invoked_parents <ext.commands.Context.invoked_parents>` to get the aliases a command's parent was invoked with (:old-issue:`1874`, :old-issue:`6462`)
- |commands| Add a converter for :class:`PartialMessage` under :class:`ext.commands.PartialMessageConverter` (:old-issue:`6308`)
- |commands| Add a converter for :class:`Guild` under :class:`ext.commands.GuildConverter` (:old-issue:`6016`, :old-issue:`6365`)
...
- |commands| Allow callable types to act as a bucket key for cooldowns (:old-issue:`6563`)
- |commands| Add ``linesep`` keyword argument to :class:`Paginator <ext.commands.Paginator>` (:old-issue:`5975`)
- |commands| Allow ``None`` to be passed to :attr:`HelpCommand.verify_checks <ext.commands.HelpCommand.verify_checks>` to only verify in a guild context (:old-issue:`2008`, :old-issue:`6446`)
- |commands| Allow relative paths when loading extensions via a ``package`` keyword argument (:old-issue:`2465`, :old-issue:`6445`)

Bug Fixes
~~~~~~~~~~

- Fix mentions not working if ``mention_author`` is passed in :meth:`abc.Messageable.send` without :attr:`Client.allowed_mentions` set (:old-issue:`6192`, :old-issue:`6458`)
- Fix user created instances of :class:`CustomActivity` triggering an error (:old-issue:`4049`)
    - Note that currently, bot users still cannot set a custom activity due to a Discord limitation.
- Fix :exc:`ZeroDivisionError` being raised from :attr:`VoiceClient.average_latency` (:old-issue:`6430`, :old-issue:`6436`)
- Fix :attr:`User.public_flags` not updating upon edit (:old-issue:`6315`)
- Fix :attr:`Message.call` sometimes causing attribute errors (:old-issue:`6390`)
- Fix issue resending a file during request retries on newer versions of ``aiohttp`` (:old-issue:`6531`)
...
- Fix potential :exc:`AttributeError` when accessing :attr:`VoiceChannel.members` (:old-issue:`6602`)
- :class:`Embed` constructor parameters now implicitly convert to :class:`str` (:old-issue:`6574`)
- Ensure ``discord`` package is only run if executed as a script (:old-issue:`6483`)
- |commands| Fix irrelevant commands potentially being unloaded during cog unload due to failure.
- |commands| Fix attribute errors when setting a cog to :class:`~.ext.commands.HelpCommand` (:old-issue:`5154`)
- |commands| Fix :attr:`Context.invoked_with <ext.commands.Context.invoked_with>` being improperly reassigned during a :meth:`~ext.commands.Context.reinvoke` (:old-issue:`6451`, :old-issue:`6462`)
- |commands| Remove duplicates from :meth:`HelpCommand.get_bot_mapping <ext.commands.HelpCommand.get_bot_mapping>` (:old-issue:`6316`)
- |commands| Properly handle positional-only parameters in bot command signatures (:old-issue:`6431`)
- |commands| Group signatures now properly show up in :attr:`Command.signature <ext.commands.Command.signature>` (:old-issue:`6529`, :old-issue:`6530`)
...
- :class:`Permission` class methods were updated to match the UI of the Discord client (:old-issue:`6476`)
- ``_`` and ``-`` characters are now stripped when making a new cog using the ``discord`` package (:old-issue:`6313`)
...
- Add support for :meth:`AppInfo.icon_url_as` and :meth:`AppInfo.cover_image_url_as` (:old-issue:`5888`)
- Add :meth:`Colour.random` to get a random colour (:old-issue:`6067`)
- Add support for stickers via :class:`Sticker` (:old-issue:`5946`)
- Add support for replying via :meth:`Message.reply` (:old-issue:`6061`)
...
- Add support for chunking an :class:`AsyncIterator` through :meth:`AsyncIterator.chunk` (:old-issue:`6100`, :old-issue:`6082`)
- Add :attr:`PartialEmoji.created_at` (:old-issue:`6128`)
- Add support for editing and deleting webhook sent messages (:old-issue:`6058`)
    - This adds :class:`WebhookMessage` as well to power this behaviour.
- Add :class:`PartialMessage` to allow working with a message via channel objects and just a message_id (:old-issue:`5905`)
    - This is useful if you don't want to incur an extra API call to fetch the message.
- Add :meth:`Emoji.url_as` (:old-issue:`6162`)
- Add support for :attr:`Member.pending` for the membership gating feature.
- Allow ``colour`` parameter to take ``int`` in :meth:`Guild.create_role` (:old-issue:`6195`)
- Add support for ``presences`` in :meth:`Guild.query_members` (:old-issue:`2354`)
- |commands| Add support for ``description`` keyword argument in :class:`commands.Cog <ext.commands.Cog>` (:old-issue:`6028`)
...
- Raise :exc:`DiscordServerError` when reaching 503s repeatedly (:old-issue:`6044`)
- Fix :exc:`AttributeError` when :meth:`Client.fetch_template` is called (:old-issue:`5986`)
- Fix errors when playing audio and moving to another channel (:old-issue:`5953`)
- Fix :exc:`AttributeError` when voice channels disconnect too fast (:old-issue:`6039`)
...
- Fix :exc:`UnboundLocalError` for editing ``public_updates_channel`` in :meth:`Guild.edit` (:old-issue:`6093`)
- Fix uninitialised :attr:`CustomActivity.created_at` (:old-issue:`6095`)
- |commands| Errors during cog unload no longer stops module cleanup (:old-issue:`6113`)
- |commands| Properly cleanup lingering commands when a conflicting alias is found when adding commands (:old-issue:`6217`)

Miscellaneous
~~~~~~~~~~~~~~~

- ``ffmpeg`` spawned processes no longer open a window in Windows (:old-issue:`6038`)
- Update dependencies to allow the library to work on Python 3.9+ without requiring build tools. (:old-issue:`5984`, :old-issue:`5970`)
- Fix docstring issue leading to a SyntaxError in 3.9 (:old-issue:`6153`)
- Update Windows opus binaries from 1.2.1 to 1.3.1 (:old-issue:`6161`)
- Allow :meth:`Guild.create_role` to accept :class:`int` as the ``colour`` parameter (:old-issue:`6195`)
- |commands| :class:`MessageConverter <ext.commands.MessageConverter>` regex got updated to support ``www.`` prefixes (:old-issue:`6002`)
...
- |commands| :func:`max_concurrency <ext.commands.max_concurrency>` is now called before cooldowns (:old-issue:`6172`)
...
- Fix :func:`utils.escape_markdown` not escaping quotes properly (:old-issue:`5897`)
- Fix :class:`Message` not being hashable (:old-issue:`5901`, :old-issue:`5866`)
- Fix moving channels to the end of the channel list (:old-issue:`5923`)
- Fix seemingly strange behaviour in ``__eq__`` for :class:`PermissionOverwrite` (:old-issue:`5929`)
- Fix aliases showing up in ``__iter__`` for :class:`Intents` (:old-issue:`5945`)
- Fix the bot disconnecting from voice when moving them to another channel (:old-issue:`5904`)
- Fix attribute errors when chunking times out sometimes during delayed on_ready dispatching.
- Ensure that the bot's own member is not evicted from the cache (:old-issue:`5949`)

Miscellaneous
~~~~~~~~~~~~~~

- Members are now loaded during ``GUILD_MEMBER_UPDATE`` events if :attr:`MemberCacheFlags.joined` is set. (:old-issue:`5930`)
...
- Add :attr:`VoiceRegion.south_korea` (:old-issue:`5233`)
- Add support for ``__eq__`` for :class:`Message` (:old-issue:`5789`)
- Add :meth:`Colour.dark_theme` factory method (:old-issue:`1584`)
- Add :meth:`AllowedMentions.none` and :meth:`AllowedMentions.all` (:old-issue:`5785`)
- Add more concrete exceptions for 500 class errors under :class:`DiscordServerError` (:old-issue:`5797`)
...
- Add support for :attr:`ActivityType.competing` (:old-issue:`5823`)
    - This seems currently unused API wise.

- Add support for message references, :attr:`Message.reference` (:old-issue:`5754`, :old-issue:`5832`)
- Add alias for :class:`ColourConverter` under ``ColorConverter`` (:old-issue:`5773`)
- Add alias for :attr:`PublicUserFlags.verified_bot_developer` under :attr:`PublicUserFlags.early_verified_bot_developer` (:old-issue:`5849`)
- |commands| Add support for ``require_var_positional`` for :class:`Command` (:old-issue:`5793`)

Bug Fixes
~~~~~~~~~~

- Fix issue with :meth:`Guild.by_category` not showing certain channels.
- Fix :attr:`abc.GuildChannel.permissions_synced` always being ``False`` (:old-issue:`5772`)
- Fix handling of cloudflare bans on webhook related requests (:old-issue:`5221`)
- Fix cases where a keep-alive thread would ack despite already dying (:old-issue:`5800`)
- Fix cases where a :class:`Member` reference would be stale when cache is disabled in message events (:old-issue:`5819`)
- Fix ``allowed_mentions`` not being sent when sending a single file (:old-issue:`5835`)
- Fix ``overwrites`` being ignored in :meth:`abc.GuildChannel.edit` if ``{}`` is passed (:old-issue:`5756`, :old-issue:`5757`)
- |commands| Fix exceptions being raised improperly in command invoke hooks (:old-issue:`5799`)
- |commands| Fix commands not being properly ejected during errors in a cog injection (:old-issue:`5804`)
- |commands| Fix cooldown timing ignoring edited timestamps.
- |tasks| Fix tasks extending the next iteration on handled exceptions (:old-issue:`5762`, :old-issue:`5763`)

Miscellaneous
~~~~~~~~~~~~~~~

- Webhook requests are now logged (:old-issue:`5798`)
...
- Usage of namedtuples has been reduced to avoid potential breaking changes in the future (:old-issue:`5834`)
- |commands| All :class:`BadArgument` exceptions from the built-in converters now raise concrete exceptions to better tell them apart (:old-issue:`5748`)
- |tasks| Lazily fetch the event loop to prevent surprises when changing event loop policy (:old-issue:`5808`)
...
- Properly terminate the connection when :meth:`Client.close` is called (:old-issue:`5207`)
- Fix error being raised when clearing embed author or image when it was already cleared (:old-issue:`5210`, :old-issue:`5212`)
- Fix ``__path__`` to allow editable extensions (:old-issue:`5213`)
...
- :class:`AutoShardedClient` has been completely redesigned from the ground up to better suit multi-process clusters (:old-issue:`2654`)
...
- Add support for guild templates (:old-issue:`2652`)
...
- Add support for guild integrations (:old-issue:`2051`, :old-issue:`1083`)
...
- Add an alias for :attr:`VerificationLevel.extreme` under :attr:`VerificationLevel.very_high` (:old-issue:`2650`)
- Add various grey to gray aliases for :class:`Colour` (:old-issue:`5130`)
- Added :attr:`VoiceClient.latency` and :attr:`VoiceClient.average_latency` (:old-issue:`2535`)
- Add ``use_cached`` and ``spoiler`` parameters to :meth:`Attachment.to_file` (:old-issue:`2577`, :old-issue:`4095`)
- Add ``position`` parameter support to :meth:`Guild.create_category` (:old-issue:`2623`)
- Allow passing ``int`` for the colour in :meth:`Role.edit` (:old-issue:`4057`)
- Add :meth:`Embed.remove_author` to clear author information from an embed (:old-issue:`4068`)
- Add the ability to clear images and thumbnails in embeds using :attr:`Embed.Empty` (:old-issue:`4053`)
- Add :attr:`Guild.max_video_channel_users` (:old-issue:`4120`)
- Add :attr:`Guild.public_updates_channel` (:old-issue:`4120`)
- Add ``guild_ready_timeout`` parameter to :class:`Client` and subclasses to control timeouts when the ``GUILD_CREATE`` stream takes too long (:old-issue:`4112`)
- Add support for public user flags via :attr:`User.public_flags` and :class:`PublicUserFlags` (:old-issue:`3999`)
- Allow changing of channel types via :meth:`TextChannel.edit` to and from a news channel (:old-issue:`4121`)
- Add :meth:`Guild.edit_role_positions` to bulk edit role positions in a single API call (:old-issue:`2501`, :old-issue:`2143`)
- Add :meth:`Guild.change_voice_state` to change your voice state in a guild (:old-issue:`5088`)
- Add :meth:`PartialInviteGuild.is_icon_animated` for checking if the invite guild has animated icon (:old-issue:`4180`, :old-issue:`4181`)
- Add :meth:`PartialInviteGuild.icon_url_as` now supports ``static_format`` for consistency (:old-issue:`4180`, :old-issue:`4181`)
- Add support for ``user_ids`` in :meth:`Guild.query_members`
- Add support for pruning members by roles in :meth:`Guild.prune_members` (:old-issue:`4043`)
- |commands| Implement :func:`~ext.commands.before_invoke` and :func:`~ext.commands.after_invoke` decorators (:old-issue:`1986`, :old-issue:`2502`)
- |commands| Add a way to retrieve ``retry_after`` from a cooldown in a command via :meth:`Command.get_cooldown_retry_after <.ext.commands.Command.get_cooldown_retry_after>` (:old-issue:`5195`)
- |commands| Add a way to dynamically add and remove checks from a :class:`HelpCommand <.ext.commands.HelpCommand>` (:old-issue:`5197`)
- |tasks| Add :meth:`Loop.is_running <.ext.tasks.Loop.is_running>` method to the task objects (:old-issue:`2540`)
- |tasks| Allow usage of custom error handlers similar to the command extensions to tasks using :meth:`Loop.error <.ext.tasks.Loop.error>` decorator (:old-issue:`2621`)


Bug Fixes
~~~~~~~~~~~~

- Fix issue with :attr:`PartialEmoji.url` reads leading to a failure (:old-issue:`4015`, :old-issue:`4016`)
- Allow :meth:`abc.Messageable.history` to take a limit of ``1`` even if ``around`` is passed (:old-issue:`4019`)
- Fix :attr:`Guild.member_count` not updating in certain cases when a member has left the guild (:old-issue:`4021`)
- Fix the type of :attr:`Object.id` not being validated. For backwards compatibility ``str`` is still allowed but is converted to ``int`` (:old-issue:`4002`)
- Fix :meth:`Guild.edit` not allowing editing of notification settings (:old-issue:`4074`, :old-issue:`4047`)
- Fix crash when the guild widget contains channels that aren't in the payload (:old-issue:`4114`, :old-issue:`4115`)
- Close ffmpeg stdin handling from spawned processes with :class:`FFmpegOpusAudio` and :class:`FFmpegPCMAudio` (:old-issue:`4036`)
- Fix :func:`utils.escape_markdown` not escaping masked links (:old-issue:`4206`, :old-issue:`4207`)
- Fix reconnect loop due to failed handshake on region change (:old-issue:`4210`, :old-issue:`3996`)
- Fix :meth:`Guild.by_category` not returning empty categories (:old-issue:`4186`)
- Fix certain JPEG images not being identified as JPEG (:old-issue:`5143`)
- Fix a crash when an incomplete guild object is used when fetching reaction information (:old-issue:`5181`)
- Fix a timeout issue when fetching members using :meth:`Guild.query_members`
- Fix an issue with domain resolution in voice (:old-issue:`5188`, :old-issue:`5191`)
- Fix an issue where :attr:`PartialEmoji.id` could be a string (:old-issue:`4153`, :old-issue:`4152`)
- Fix regression where :attr:`Member.activities` would not clear.
- |commands| A :exc:`TypeError` is now raised when :obj:`typing.Optional` is used within :data:`commands.Greedy <.ext.commands.Greedy>` (:old-issue:`2253`, :old-issue:`5068`)
- |commands| :meth:`Bot.walk_commands <.ext.commands.Bot.walk_commands>` no longer yields duplicate commands due to aliases (:old-issue:`2591`)
- |commands| Fix regex characters not being escaped in :attr:`HelpCommand.clean_prefix <.ext.commands.HelpCommand.clean_prefix>` (:old-issue:`4058`, :old-issue:`4071`)
- |commands| Fix :meth:`Bot.get_command <.ext.commands.Bot.get_command>` from raising errors when a name only has whitespace (:old-issue:`5124`)
- |commands| Fix issue with :attr:`Context.subcommand_passed <.ext.commands.Context.subcommand_passed>` not functioning as expected (:old-issue:`5198`)
- |tasks| Task objects are no longer stored globally so two class instances can now start two separate tasks (:old-issue:`2294`)
- |tasks| Allow cancelling the loop within :meth:`before_loop <.ext.tasks.Loop.before_loop>` (:old-issue:`4082`)


Miscellaneous
~~~~~~~~~~~~~~~

- The :attr:`Member.roles` cache introduced in v1.3 was reverted due to issues caused (:old-issue:`4087`, :old-issue:`4157`)
- :class:`Webhook` objects are now comparable and hashable (:old-issue:`4182`)
- Some more API requests got a ``reason`` parameter for audit logs (:old-issue:`5086`)
...
- Reduce memory usage when reconnecting due to stale references being held by the message cache (:old-issue:`5133`)
- Optimize :meth:`abc.GuildChannel.permissions_for` by not creating as many temporary objects (20-32% savings).
- |commands| Raise :exc:`~ext.commands.CommandRegistrationError` instead of :exc:`ClientException` when a duplicate error is registered (:old-issue:`4217`)
- |tasks| No longer handle :exc:`HTTPException` by default in the task reconnect loop (:old-issue:`5193`)
...
- Fix an issue with channel overwrites causing multiple issues including crashes (:old-issue:`5109`)
...
- Fix ``None`` appearing in ``Member.activities``. (:old-issue:`2619`)
...
- Fix various Cloudflare handling errors (:old-issue:`2572`, :old-issue:`2544`)
...
- Fix invalid format specifier in webhook state (:old-issue:`2570`)
...
- Fix the channel returned from :meth:`Client.fetch_channel` raising when sending messages. (:old-issue:`2531`)
...
- Add :meth:`Guild.fetch_members` to fetch members from the HTTP API. (:old-issue:`2204`)
- Add :meth:`Guild.fetch_roles` to fetch roles from the HTTP API. (:old-issue:`2208`)
- Add support for teams via :class:`Team` when fetching with :meth:`Client.application_info`. (:old-issue:`2239`)
...
- Add :class:`FFmpegOpusAudio` and other voice improvements. (:old-issue:`2258`)
- Add :attr:`RawMessageUpdateEvent.channel_id` for retrieving channel IDs during raw message updates. (:old-issue:`2301`)
- Add :attr:`RawReactionActionEvent.event_type` to disambiguate between reaction addition and removal in reaction events.
- Add :attr:`abc.GuildChannel.permissions_synced` to query whether permissions are synced with the category. (:old-issue:`2300`, :old-issue:`2324`)
- Add :attr:`MessageType.channel_follow_add` message type for announcement channels being followed. (:old-issue:`2314`)
- Add :meth:`Message.is_system` to allow for quickly filtering through system messages.
- Add :attr:`VoiceState.self_stream` to indicate whether someone is streaming via Go Live. (:old-issue:`2343`)
- Add :meth:`Emoji.is_usable` to check if the client user can use an emoji. (:old-issue:`2349`)
- Add :attr:`VoiceRegion.europe` and :attr:`VoiceRegion.dubai`. (:old-issue:`2358`, :old-issue:`2490`)
- Add :meth:`TextChannel.follow` to follow a news channel. (:old-issue:`2367`)
- Add :attr:`Permissions.view_guild_insights` permission. (:old-issue:`2415`)
- Add support for new audit log types. See :ref:`discord-api-audit-logs` for more information. (:old-issue:`2427`)
    - Note that integration support is not finalized.

- Add :attr:`Webhook.type` to query the type of webhook (:class:`WebhookType`). (:old-issue:`2441`)
- Allow bulk editing of channel overwrites through :meth:`abc.GuildChannel.edit`. (:old-issue:`2198`)
- Add :class:`Activity.created_at` to see when an activity was started. (:old-issue:`2446`)
- Add support for ``xsalsa20_poly1305_lite`` encryption mode for voice. (:old-issue:`2463`)
- Add :attr:`RawReactionActionEvent.member` to get the member who did the reaction. (:old-issue:`2443`)
- Add support for new YouTube streaming via :attr:`Streaming.platform` and :attr:`Streaming.game`. (:old-issue:`2445`)
- Add :attr:`Guild.discovery_splash_url` to get the discovery splash image asset. (:old-issue:`2482`)
- Add :attr:`Guild.rules_channel` to get the rules channel of public guilds. (:old-issue:`2482`)
    - It should be noted that this feature is restricted to those who are either in Server Discovery or planning to be there.

- Add support for message flags via :attr:`Message.flags` and :class:`MessageFlags`. (:old-issue:`2433`)
...
- Add certain aliases to :class:`Permissions` to match the UI better. (:old-issue:`2496`)
...
- Add support for custom activities via :class:`CustomActivity`. (:old-issue:`2400`)
...
- Add :func:`utils.sleep_until` helper to sleep until a specific datetime. (:old-issue:`2517`, :old-issue:`2519`)
- |commands| Add support for teams and :attr:`Bot.owner_ids <.ext.commands.Bot.owner_ids>` to have multiple bot owners. (:old-issue:`2239`)
- |commands| Add new :attr:`BucketType.role <.ext.commands.BucketType.role>` bucket type. (:old-issue:`2201`)
- |commands| Expose :attr:`Command.cog <.ext.commands.Command.cog>` property publicly. (:old-issue:`2360`)
- |commands| Add non-decorator interface for adding checks to commands via :meth:`Command.add_check <.ext.commands.Command.add_check>` and :meth:`Command.remove_check <.ext.commands.Command.remove_check>`. (:old-issue:`2411`)
- |commands| Add :func:`has_guild_permissions <.ext.commands.has_guild_permissions>` check. (:old-issue:`2460`)
- |commands| Add :func:`bot_has_guild_permissions <.ext.commands.bot_has_guild_permissions>` check. (:old-issue:`2460`)
...
- |tasks| :meth:`Loop.add_exception_type <.ext.tasks.Loop.add_exception_type>` now allows multiple exceptions to be set. (:old-issue:`2333`)
- |tasks| Add :attr:`Loop.next_iteration <.ext.tasks.Loop.next_iteration>` property. (:old-issue:`2305`)

Bug Fixes
~~~~~~~~~~

- Fix issue with permission resolution sometimes failing for guilds with no owner.
- Tokens are now stripped upon use. (:old-issue:`2135`)
- Passing in a ``name`` is no longer required for :meth:`Emoji.edit`. (:old-issue:`2368`)
- Fix issue with webhooks not re-raising after retries have run out. (:old-issue:`2272`, :old-issue:`2380`)
- Fix mismatch in URL handling in :func:`utils.escape_markdown`. (:old-issue:`2420`)
- Fix issue with ports being read in little endian when they should be big endian in voice connections. (:old-issue:`2470`)
...
- Fix :func:`on_shard_ready` not triggering when ``fetch_offline_members`` is disabled. (:old-issue:`2504`)
...
- |commands| Extensions that fail internally due to ImportError will no longer raise :exc:`~.ext.commands.ExtensionNotFound`. (:old-issue:`2244`, :old-issue:`2275`, :old-issue:`2291`)
- |commands| Updating the :attr:`Paginator.suffix <.ext.commands.Paginator.suffix>` will not cause out of date calculations. (:old-issue:`2251`)
- |commands| Allow converters from custom extension packages. (:old-issue:`2369`, :old-issue:`2374`)
- |commands| Fix issue with paginator prefix being ``None`` causing empty pages. (:old-issue:`2471`)
...
- |commands| :attr:`HelpCommand.clean_prefix <.ext.commands.HelpCommand.clean_prefix>` now takes into consideration nickname mentions. (:old-issue:`2489`)
...
- Bump the dependency of ``websockets`` to 8.0 for those who can use it. (:old-issue:`2453`)
...
- The performance of :attr:`Guild.default_role` has been improved from O(N) to O(1). (:old-issue:`2375`)
...
- |commands| Usernames prefixed with @ inside DMs will properly convert using the :class:`User` converter. (:old-issue:`2498`)
- |tasks| The task sleeping time will now take into consideration the amount of time the task body has taken before sleeping. (:old-issue:`2516`)
...
- Fix a crash that would trigger during message updates (:old-issue:`2265`, :old-issue:`2287`).
- Fix a bug when :meth:`VoiceChannel.connect` would not return (:old-issue:`2274`, :old-issue:`2372`, :old-issue:`2373`, :old-issue:`2377`).
- Fix a crash relating to token-less webhooks (:old-issue:`2364`).
- Fix issue where :attr:`Guild.premium_subscription_count` would be ``None`` due to a Discord bug. (:old-issue:`2331`, :old-issue:`2376`).
...
- Fix an AttributeError when accessing :attr:`Member.premium_since` in :func:`on_member_update`. (:old-issue:`2213`)
- Handle :exc:`asyncio.CancelledError` in :meth:`abc.Messageable.typing` context manager. (:old-issue:`2218`)
- Raise the max encoder bitrate to 512kbps to account for nitro boosting. (:old-issue:`2232`)
- Properly propagate exceptions in :meth:`Client.run`. (:old-issue:`2237`)
...
- Add :attr:`VoiceRegion.india`. (:old-issue:`2145`)
- Add :meth:`Embed.insert_field_at`. (:old-issue:`2178`)
- Add a ``type`` attribute for all channels to their appropriate :class:`ChannelType`. (:old-issue:`2185`)
- Add :meth:`Client.fetch_channel` to fetch a channel by ID via HTTP. (:old-issue:`2169`)
- Add :meth:`Guild.fetch_channels` to fetch all channels via HTTP. (:old-issue:`2169`)
...
- |tasks| Add :meth:`Loop.change_interval <.ext.tasks.Loop.change_interval>` to change the sleep interval at runtime (:old-issue:`2158`, :old-issue:`2162`)
...
- |tasks| The decorator interface now works as expected when stacking (:old-issue:`2154`)
...
- Keyword arguments in :meth:`Client.start` and :meth:`Client.run` are now validated (:old-issue:`953`, :old-issue:`2170`)
...
- Webhooks do not overwrite data on retrying their HTTP requests (:old-issue:`2140`)
...
- Add :attr:`Permissions.stream` permission. (:old-issue:`2077`)
- Add equality comparison and hash support to :class:`Asset`
- Add ``compute_prune_members`` parameter to :meth:`Guild.prune_members` (:old-issue:`2085`)
- Add :attr:`Client.cached_messages` attribute to fetch the message cache (:old-issue:`2086`)
- Add :meth:`abc.GuildChannel.clone` to clone a guild channel. (:old-issue:`2093`)
- Add ``delay`` keyword-only argument to :meth:`Message.delete` (:old-issue:`2094`)
- Add support for ``<:name:id>`` when adding reactions (:old-issue:`2095`)
- Add :meth:`Asset.read` to fetch the bytes content of an asset (:old-issue:`2107`)
- Add :meth:`Attachment.read` to fetch the bytes content of an attachment (:old-issue:`2118`)
...
- Allow passing ``cls`` in the :func:`~.commands.group` decorator (:old-issue:`2061`)
- Add :attr:`.Command.parents` to fetch the parents of a command (:old-issue:`2104`)
...
- Tighten constraints of ``__eq__`` in :class:`Spotify` objects (:old-issue:`2113`, :old-issue:`2117`)
...
- Fix the default :func:`on_command_error` to work with new-style cogs (:old-issue:`2094`)
- DM channels are now recognised as NSFW in :func:`~.commands.is_nsfw` check.
- Fix race condition with help commands (:old-issue:`2123`)
- Fix cog descriptions not showing in :class:`~.commands.MinimalHelpCommand` (:old-issue:`2139`)
...
- Custom exception classes are now used for all default checks in the library (:old-issue:`2101`)
...
- Fix voice handshake race condition (:old-issue:`2056`, :old-issue:`2063`)
...
- Event loop is now properly propagated (:old-issue:`420`)
...
- Paginator pages do not exceed their max_size anymore (:old-issue:`340`)
...
- Fix bug that leaked keep alive threads (:old-issue:`309`)
